...
% Updated position descriptions to match window
...
% Define the shifted window center (same as used for pursuit)
shifted_center = [scr.x_mid, const.disp_margin_top + const.disp_max/2];

% The coordinate calculation - FIXED to use shifted center
const.fixation_coord_x = linspace(shifted_center(1) - const.window_size/2, ...
                                  shifted_center(1) + const.window_size/2, ...
                                  const.fixation_cols);
const.fixation_coord_y = linspace(shifted_center(2) - const.window_size/2, ...
                                  shifted_center(2) + const.window_size/2, ...
                                  const.fixation_rows);  % FIXED: should be rows, not cols
...
const.pursuit_amps_txt = {'4.0 dva', '5.0 dva', '6.0 dva'};
